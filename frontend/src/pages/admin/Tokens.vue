...
const sortedTokens = computed(() => {
    if (tokens) {
        ...
        function compare(a: OutputStaticTokenInfo, b: OutputStaticTokenInfo) {
            ...
            if (a_created_date < b_created_date)
                return -1;
            if (a_created_date > b_created_date)
                return 1;
            return 0;
...
const generateToken = async () => {
    var outRequest = {} as NewTokenRequestBody
...
<template>
    ...
    <section class="flex flex-col gap-4">
        <div class="row flex flex-col sm:flex-row gap-4">
            <VaCard>
                ...
                <VaCardContent>
                    <div>
                        <VaButton size="small" icon="add" style="margin-bottom: 1rem;" color="gsgYellow" round
                            @click="showModalAdd">
                            Generate New Token
                        </VaButton>
                    </div>
                        ...
                        <table class="va-table">
                            ...
                            <tr v-for="token in sortedTokens" :key="token.id" rowspan="2">
                                <template v-if="showRevoked || showRevoked == token.revoked">
                                    <td>
                                        <VaIcon v-if="!token.revoked && currentDate < new Date(token.expires_at)" name="check" color="#00ff00"></VaIcon>
                                    ...
                                    <td>{{ token.purpose }}</td>
                                    <td>
                                        <VaButton v-if="!token.revoked" size="small" round icon="stop_circle" color="gsgRed"
                                        @click="showModalRevoke(token.id)">Revoke</VaButton>
                                    </td>
                                </template>
                            </tr>
                        </table>
    ...
    <VaModal v-model="addModalShow" hide-default-actions blur>
        ...
        <div class="flex flex-col items-start gap-2">
            <VaForm ref="addForm" immediate hide-error-messages class="flex flex-col gap-2 mb-2">
                <VaTextarea v-model="addModalForm.purpose" label="Purpose" name="StaticTokenPurpose"
                    placeholder="describe how this token will be used">
                </VaTextarea>
                <VaCheckbox v-model="expiresCheckbox" label="Set an expiry date"></VaCheckbox>
                <VaDateInput v-model="addModalForm.expires_at" v-if="expiresCheckbox" name="StaticTokenExpiryDate"
                    label="Expiry Date" :rules="dateValidationRules"></VaDateInput>
    ...
    <VaModal v-model="tokenRevealShow" ok-text="OK" size="small" hide-default-actions blur noDismiss>
        <p>Your new token is:</p>
        <VaTextarea style="width: 100%" autosize minRows="6" readonly v-model="StaticTokenStore.$state.newToken.token">
        </VaTextarea>
        <p>This is the only time you will be shown this token.</p>
        <template #footer>
            <div class="flex gap-2">
                <VaButton color="gsgYellow"
                    @click="addModalShow = false; tokenRevealShow = false; StaticTokenStore.clearNewToken()">OK
                </VaButton>
            </div>
        </template>
    </VaModal>
    <VaModal v-model="revokeModalShow" hide-default-actions blur ok-text="Apply">
        ...
        <p>
            Are you sure you want to revoke the selected token?
        </p>
        <template #footer>
            <div class="flex gap-2">
